...
class Data:
    def __init__(self, seq, data, is_ack):
        self.seq = seq
        self.data = data
        self.is_ack = is_ack
...
class GBNSender:
    def __init__(self):
        ...
        self.receiver_address = ('127.0.0.1', 7777)   # 接收方地址
...
def main(sender_socket):
    ...
    while True:
        rs, ws, es = select.select([sender_socket.socket, ], [], [], 1)
        if len(rs) > 0:
            message, address = sender_socket.socket.recvfrom(sender_socket.buffer_size)
